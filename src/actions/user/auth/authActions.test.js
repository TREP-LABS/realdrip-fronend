...
describe("Auth Actions", () => {
  ...
  describe("Login admin user", () => {
    it("should dispatch ADDING_USER_STARTED and ADDING_USER_SIGNIN", async done => {
      const samplePayload = { data: {}, message: "Loginin user successfully" };
      moxios.wait(() => {
        const request = moxios.requests.mostRecent();
        request.respondWith({ response: samplePayload });
      });
      const expectedActions = ["ADDING_USER_STARTED", "ADDING_USER_SIGNIN"];
      const expectedPayloads = [undefined, samplePayload];
      const store = mockStore({});
      await store.dispatch(actions.loginAdminUser()).then(() => {
        const dispatchedActions = store.getActions();
        const actionTypes = dispatchedActions.map(action => action.type);
        const payloads = dispatchedActions.map(action => action.payload);
        expect(actionTypes).toEqual(expectedActions);
